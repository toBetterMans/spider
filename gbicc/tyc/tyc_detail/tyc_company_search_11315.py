...
# proxies = {'http': 'http://H017W84J271F18FD:451D275088D7ABD3@proxy.abuyun.com:9020'}
# proxies = {'http': 'http://H017W84J271F18FD:451D275088D7ABD3@proxy.abuyun.com:9020'}
...
class DetailSpider(object):
    ...
    def get_company_list(self, number, table, company_name, branch):
        ...
        if con.status_code != 403:
            ...
            try:
                ...
                self.parse_company_list(key, con.text, table, branch)
            except Exception as e:
                self.mysqlClient.mysql_update(table, {'searched': 1, 'error': 1}, {'number': number})
                logger.exception("Exception Logged")
        else:
            logger.info("IP is forbid!")
            time.sleep(60 * 60)
            self.get_company_list(key, table, company_name, branch)

    def parse_company_list(self, key, text, table, branch):
        """
        ...
        if div_first:
            ...
            try:
                ...
                # 第一行
                home_message_one = div_first.find('div', class_="search_right_item").find('div')
                # 第二行
                home_message_two = div_first.find('div', class_="search_row_new").find_all('div',
                                                                                       class_="title overflow-width")

                if home_message_two:
                    pass
                try:
                    # 法定代表人
                    legal_representative = home_message_two[0].find('a').text.strip()
                except AttributeError:
                    legal_representative =home_message_two[0].find('span').text.strip()
                #     注册资本
                registered_capital = home_message_two[1].find('span').text.strip()
                # 注册时间
                registration_date = home_message_two[2].find('span').text.strip()
                # 位置
                location =home_message_one.find('span',class_='float-right search-right-center').text.strip()
                try:
                    # 评分
                    score_div =div_first.find('div',class_='in-block vertical-middle float-right search-right-center')
                    score_div_spans=score_div.find_all('span')
                    score=score_div_spans[0].text.strip()
                except IndexError:
                    score = ""
                #     营业状态
                status_type = home_message_one.find('div',class_="statusTypeNor").text.strip()

                # 抓取公司详细信息
                mongo_result = self.get_detail(ent_name, url, branch)
                if mongo_result is None:
                    return None

                txt_id = mongo_result['_id']
                add_time = str(datetime.datetime.now())
                value_list = [key, ent_name, legal_representative, registered_capital, registration_date, location,
                              score, status_type, url, str(txt_id), add_time, str(0), branch]
                column_name = "(search_name,ent_name,legal_representative,registered_capital,registration_date,location,score,status_type,url,txt_id,add_time,mark,branch)"
                ...
                self.mysqlClient.mysql_insert(CompanyBasicInfo.table_name, column_name, insert_value)
                self.mysqlClient.mysql_update(table, {'searched': 1, 'error': 0}, {'number': number})
            except Exception as e:
                self.mysqlClient.mysql_update(table, {'searched': 1, 'error': 1}, {'number': number})
            ...
            self.mysqlClient.mysql_update(table, {'searched': 1, 'error': 2}, {'number': number})

    def get_detail(self, ent_name, url, branch):
        """
        ...
        :return:
        """
        res = {"ent_name": ent_name, "url": url, "branch": branch}
        ...
        if con.status_code != 403:
            ...
            res["parse"] = 0
            res["error_list"] = ""
            res["addTime"] = datetime.datetime.now() + datetime.timedelta(hours=8)

            # 公司详细信息页面存入mongodb
            logger.info("Save detail html to mongodb %s", ent_name)
            mongo_result = self.mongodbClient.mongodb_insert("company_detail_info", res)

            return mongo_result
        else:
            logger.info("IP is forbid!")
            time.sleep(60 * 60)
            self.get_detail(ent_name, url, branch)
...
if __name__ == "__main__":
    ...
    while True:
        try:
            result = ds.mysqlClient.mysql_find_one(table, parameters=parameters)
            if result:
                number = result[0]
                company_name = result[2]
                branch = result[14]
                ds.get_company_list(number, table, company_name, branch)
            else:
        ...
        if ds.count == 45:
            ...
            ds.mysqlClient.mysql_update("tyc_user", update_parameters, where_parameters)
            ds.login()
