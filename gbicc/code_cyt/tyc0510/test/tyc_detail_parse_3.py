...
class TycDetailParse(object):
        ...
        if top:
            ...
            if table_lists:
                ...
                if trs0 and trs1:
                    ...
                    print(baseinfo.registerFund)
                    # 企业状态(经营状态)
                    baseinfo.companyStatus = trs1[1].xpath('./td[4]//text()')[0]
                    print(baseinfo.companyStatus)
                    # 工商注册号
                    baseinfo.registerNum = trs1[2].xpath('./td[4]//text()')[0]
                    print(baseinfo.registerNum)
                    # 组织机构代码
                    baseinfo.tissueNum = trs1[3].xpath('./td[position()=4]/text()')[0]
                    print(baseinfo.tissueNum)
                    # 统一社会信用代码
                    baseinfo.creditNum = trs1[2].xpath('./td[position()=2]/text()')[0]
                    print(baseinfo.creditNum)
                    # 企业类型(公司类型)
                    baseinfo.companyType = trs1[4].xpath('./td[2]')[0].xpath('string(.)')
                    print(baseinfo.companyType)
                    # 纳税人识别号
                    baseinfo.taxpayerNum = trs1[3].xpath('./td[2]')[0].xpath('string(.)')
                    print(baseinfo.taxpayerNum)
                    # 行业
                    baseinfo.industry = trs1[4].xpath('./td[4]')[0].xpath('string(.)')
                    print(baseinfo.industry)
                    # 营业期限
                    businessTerm = trs1[6].xpath('./td[2]')[0].xpath('string(.)')
                    print(businessTerm)
                    ...
                    # 注册时间
                    registerDate = try_and_text("variable[1].xpath('./td[2]')[0].xpath('string(.)')", trs1)
                    print(registerDate)
                    # 新增 纳税人资质
                    taxQualificate = try_and_text("variable[6].xpath('./td[4]')[0].xpath('string(.)')", trs1)
                    baseinfo.taxQualificate = taxQualificate[0] if taxQualificate else 'NA'
                    print(baseinfo.taxQualificate)
                    # 核准日期
                    checkDate = try_and_text("variable[5].xpath('./td[2]')[0].xpath('string(.)')", trs1)
                    print(checkDate)
                    ...
                    # 人员规模
                    baseinfo.persionSize = try_and_text("variable[7].xpath('./td[2]')[0].xpath('string(.)')", trs1)
                    # 实缴资本：
                    baseinfo.paidCapital = try_and_text("variable[0].xpath('./td[4]')[0].xpath('string(.)')", trs1)
                    print(baseinfo.paidCapital)
                    # 参保人数：
                    baseinfo.insuredPersion = try_and_text("variable[7].xpath('./td[4]')[0].xpath('string(.)')", trs1)
                    # 登记机关
                    baseinfo.registerOffice = try_and_text("variable[5].xpath('./td[4]')[0].xpath('string(.)')", trs1)
                    baseinfo.txtId = self.txt_id or 'NA'
    ...
    def html_parse_mainPerson(self):
        ...
        if trs:
            ...
            for div in trs:
                ...
                mainPerson.position = ''.join(position).replace('\n', '').replace("'", '') if position else 'NA'
                name = div.xpath('.//tr/td[2]/a/text()')
                try:
    ...
    def html_parse_shareholderInfo(self, index):
        ...
        if trs:
            ...
            for tr in trs:
                ...
                shareholderInfo.fundTime = CURRENT_VERSION_NULL
    ...
    def html_parse_investInfo(self, index):
        ...
        if trs:
            ...
            for tr in trs:
                ...
                investInfo.investAmount = '页面上无此信息'
                # 投资占比
                investInfo.investRatio = try_and_text("variable[5].xpath('.//text()')[0]", tds)
                # 注册时间
                investInfo.investDate = try_and_text("variable[3].xpath('.//text()')[0]", tds)
                # 状态
                investInfo.investStatus = try_and_text("variable[6].xpath('.//text()')[0]", tds)
                # TODO:627新增 关联产品
                investInfo.related_products = try_and_text("variable[7].xpath('.//text()')[0]", tds)
                # TODO:627新增 关联机构
                investInfo.associated_agencies = try_and_text("variable[8].xpath('.//text()')[0]", tds)
                ...
                value_list = [
                    ...
                    investInfo.related_products,
                    investInfo.associated_agencies,
                    investInfo.mark,
    ...
    def html_parse_alterRecord(self, index):
        ...
        if trs:
            ...
            for tr in trs:
                ...
                # TODO: 627
                alterProject = try_and_text("variable[2].xpath('./div/text()')", tds)
    ...
    def html_parse_year_wzhwdxx(self, year_selector, year):
        ...
        year_selector = year_selector.replace('网站或网店信息', 'year_wangzhan')
        if year_selector:
            year_selector = etree.HTML(year_selector)
            ...
            if trs:
                ...
                for tr in trs:
                    insert_value = ""
                    tds = tr.xpath("./td")
                    website.website_type = try_and_text('variable[1].xpath(".//text()")[0]', tds)
                    website.web_name = try_and_text('variable[2].xpath("./div/text()")[0]', tds)
                    web_url = try_and_text('variable[3].xpath("./div/a/text()")', tds)
                    logger.debug(
                        'web_url={} {}'.format(
    ...
    def html_parse_year_gdczxx(self, year_selector, year):
        ...
        # 获得分支机构大标签
        year_selector = year_selector.replace('股东及出资信息', 'year_gudongchuzi')
        if year_selector:
            year_selector = etree.HTML(year_selector)
        trs = year_selector.xpath(
        ...
        if trs:
            ...
            for tr in trs:
                ...
                gdcz.shareholder = try_and_text("variable[1].xpath('.//tr/td/a/text()')[0]", tds)
                gdcz.subscirbe_contribution = try_and_text("variable[2].xpath('./text()')[0]", tds)

                gdcz.contribution_time = try_and_text("variable[3].xpath('./text()')[0]", tds)
                gdcz.contribution_style = try_and_text("variable[4].xpath('./text()')[0]", tds)
                actual_contribution = try_and_text("variable[5].xpath('./text()')", tds)
                gdcz.actual_contribution = 'NA'
                if actual_contribution:
                    gdcz.actual_contribution = actual_contribution[0]
                gdcz.actual_time = try_and_text("variable[6].xpath('./text()')[0]", tds)
                gdcz.actual_style = try_and_text("variable[7].xpath('./text()')[0]", tds)
                gdcz.year = year
    ...
    def html_parse_year_zczk(self, year_selector, year):
        ...
        if trs:
            ...
            flss.total_assets = replace_special_string(
                try_and_text('''variable[0].xpath("./td[position()=2]/text()")[0]''', trs))
            flss.total_income = replace_special_string(
                try_and_text('''variable[0].xpath("td[position()=4]/text()")[0]''', trs))
            flss.total_sales = replace_special_string(
                try_and_text('''variable[1].xpath("td[position()=2]/text()")[0]''', trs))
            flss.total_profit = replace_special_string(
                try_and_text('''variable[1].xpath("td[position()=4]/text()")[0]''', trs))
            flss.operation_income = replace_special_string(
                try_and_text('''variable[2].xpath("td[position()=2]/text()")[0]''', trs))
            flss.net_profit = replace_special_string(
                try_and_text('''variable[2].xpath("td[position()=4]/text()")[0]''', trs))
            flss.total_tax = replace_special_string(
                try_and_text('''variable[3].xpath("td[position()=2]/text()")[0]''', trs))
            flss.total_debt = replace_special_string(
                try_and_text('''variable[3].xpath("td[position()=4]/text()")[0]''', trs))
    ...
    def html_parse_branch(self, index):
        ...
        if root_div:
            ...
            for tr in trs:
                ...
                # TODO： 627
                legal_representative = try_and_text("variable[2].xpath('.//tr//td[2]/a/text()')", tds)
                logger.debug(
                    '负责人={} type={}'.format(
        ...
        if trs:
            ...
            for tr in trs:
                ...
                value_list = [
                    ...
                    ktggInfo.txt_id]
                # (detail,defendant,plaintiff,reference_num,cause_action,trial_date,batch,agency_name,agency_num,mark,company_name,txt_id,add_time)
        ...
        if root_div:
            ...
            for tr in root_div:
                ...
                if tds:
                    ...
                    flss.judgment_name = try_and_text("variable[2].xpath('./text()')[0]", tds)
                    flss.judgment_document = try_and_text("variable[2].xpath('./text()')[0]", tds)
                    ...
                    tds_href = try_and_text("variable[6].xpath('./a/@href')[0]", tds)
                    flss.document_url = tds_href if tds_href else 'NA'

                    flss.company_name = key
                    flss.add_time = 'sysdate'
                    flss.mark = 0
                    flss.detail_status = 0
                    flss.agency_num = self.agency_num
                    flss.agency_name = self.agency_name
                    # 区别key
                    flss.batch = self.batch
                    text_info = 'NA'
                    try:
                        text_info = self.detail_info['_container_lawsuit'][tds_href.split(r'/')[-1]]
                    ...
                    insert_sql = create_insert_sql(flss.table_name, flss.column_name, len(flss.column_name.split(',')))
                    single_oracle.oracle_insert_sql_param(insert_sql, value_list)
    ...
    def html_parse_announcement(self, index):
        ...
        if root_div:
            ...
            for tr in trs:
                ...
                text_info = try_and_text('''variable[6].xpath('./script/text()')[0].replace("'", '')''', tds)
    ...
    def html_parse_shixinren(self, index):
        ...
        if root_div:
            ...
            for tr in trs:
                ...
                tds = tr.xpath("./td")
                # TODO: 627 发布日期
                publication_date = try_and_text("variable[1].xpath('.//text()')", tds)

                case_date = try_and_text("variable[2].xpath('.//text()')", tds)
                case_number = try_and_text("variable[3].xpath('.//text()')", tds)
                execution_court = try_and_text("variable[4].xpath('.//text()')", tds)
                performance_state = try_and_text("variable[5].xpath('.//text()')", tds)
                execute_number = try_and_text("variable[6].xpath('.//text()')", tds)

                flss.publication_date = publication_date[0] if case_date else 'NA'
                ...
                href = try_and_text("variable[7].xpath('./span/@onclick')[0]", tds)
    ...
    def html_parse_executed(self, index):
        # TODO 新增被执行人详情    onclick事件，需要分页抓取
    ...
    def html_parse_sfxz(self, index):
        ...
        if trs:
            ...
            for tr in trs:
                ...
                tds = tr.xpath('./td')
                # 被执行人
                sfxzInfo.enforcementPerson = try_and_text("variable[1].xpath('.a/text()')[0]", tds)
                # 股权数额
    ...
    def html_parse_abnormal(self, index):
        ...
        if index == 1 and not isinstance(self.selector, int):
            ...
            # 获得经营异常大标签
            root_div = self.selector.xpath(
                '//div[@id= "_container_abnormalRemove"]/table/tbody/tr')
        if root_div:
            flss = TycJyfxJyyc()
            key = self.search_name
            # 一行是一个tr
    ...
    def html_parse_xingzhengchufa(self, index):
        ...
        if root_div:
            ...
            root_div = root_div[0]
            trs = root_div.xpath("./tbody/tr")
            # 序号	处罚决定日期	决定书文号	行政处罚内容	决定机关	操作
            for tr in trs:
                insert_value = ""
                tds = tr.xpath("./td")
                try:
    ...
    def html_parse_illegalSerious(self):
        ...
        trs = self.selector.xpath(
            '//div[@id="_container_illegalPut"]/table/tbody/tr')
        if trs:

            key = self.search_name
            # 序号	列入日期	列入决定机关	列入严重违法失信企业名单原因
            for tr in trs:
                insert_value = ""
    ...
    def html_parse_pledge(self, index):
        ...
        if root_div:
            ...
            flss = TycJyfxGqcz()
            key = self.search_name
            # 序号	股权出质设立登记日期	登记编号	出质人	质权人	状态	出质股权数额	操作
            for tr in root_div:
                insert_value = ""
                tds = tr.xpath("./td")
    ...
    def html_parse_dongchandiya(self, index):
        ...
        if root_div:
            ...
            key = self.search_name
            # 一行是一个tr
            # 序号	登记日期	登记编号	被担保债权类型	被担保债权数额	登记机关	状态	操作
            for tr in root_div:
                flss = tycJyfxDcdy()
                insert_value = ""
                tds = tr.xpath('./td')
    ...
    def html_parse_taxesNotice(self, index):
        ...
        if trs:
            ...
            # 序号	发布日期	纳税人识别号	欠税税种	当前新发生的欠税余额	欠税余额	税务机关	操作
            for tr in trs:
                insert_value = ""
                tds = tr.xpath('./td')
                taxesNotice.taxesDate = try_and_text(
                    "variable[1].xpath('text()')[0]", tds)
                taxesNotice.taxesNum = try_and_text(
                    "variable[2].xpath('text()')[0]", tds)
                taxesNotice.taxesType = try_and_text(
                    "variable[3].xpath('text()')[0]", tds)
                taxesNotice.taxesMoney = try_and_text(
                    "variable[4].xpath('text()')[0]", tds)
                taxesNotice.taxesBalance = try_and_text(
                    "variable[5].xpath('text()')[0]", tds)
                taxesNotice.taxesOffice = try_and_text(
                    "variable[6].xpath('text()')[0]", tds)
                # 新增 详情
    ...
    def html_parse_sfpm(self, index):
        ...
        if trs:
            ...
            key = self.search_name
            # 序号	公告日期	拍卖公告	拍卖标的	执行法院	操作
            for tr in trs:
                insert_value = ""
                tds = tr.xpath('./td')
                # 拍卖公告
                sfpaInfo.auctionNotice = try_and_text(
                    "variable[2].xpath('./a/text()')[0]", tds)
                # 公告日期
                sfpaInfo.auctionDate = try_and_text(
                    "variable[1].xpath('./text()')[0]", tds)
                # 执行法院
                sfpaInfo.executeCourt = try_and_text(
                    "variable[4].xpath('./text()')[0]", tds)
                # 拍卖标的
                sfpaInfo.auctionTarget = try_and_text(
    ...
    def html_parse_gscg(self):
        ...
        if trs:
            ...
            key = self.search_name
            # 序号	票据号	票据类型	票面金额	发布机构	公告日期	操作
            for tr in trs:
                insert_value = ""
                tds = tr.xpath('./td')
                # 票据号
                gscgInfo.billNumber = try_and_text(
                    "variable[1].xpath('.//text()')[0]", tds)
                # 票据类型
                gscgInfo.billType = try_and_text(
                    "variable[2].xpath('.//text()')[0]", tds)
                # 票面金额
                gscgInfo.denomination = try_and_text(
                    "variable[3].xpath('.//text()')[0]", tds)
                # 发布机构
                gscgInfo.publishAuthority = try_and_text(
                    "variable[4].xpath('.//text()')[0]", tds)
                # 公告日期
                gscgInfo.announcementDate = try_and_text(
                    "variable[5].xpath('.//text()')[0]", tds)
                # 详情
                text_info = try_and_text(
                    "variable[6].xpath('.//text()')[0]", tds)
    ...
    def html_parse_financeHistory(self):
        ...
        if trs:
            ...
            for tr in trs:
                insert_value = ""
                tds = tr.xpath('./td')
                financeHistory.financeDate = try_and_text(
                    "variable[1].xpath('string(.)')", tds)
                financeHistory.event_date = try_and_text(
                    "variable[2].xpath('string(.)')", tds)
                financeHistory.financeRound = try_and_text(
                    "variable[4].xpath('string(.)')", tds)
                financeHistory.financeValue = try_and_text(
                    "variable[5].xpath('string(.)')", tds)
                financeHistory.financeMoney = try_and_text(
                    "variable[3].xpath('string(.)')", tds)
                financeHistory.financeRatio = try_and_text(
                    "variable[6].xpath('string(.)')", tds)
                financeHistory.financeInvestor = try_and_text(
    ...
    def html_parse_coreTeam(self, index):
        ...
        if trs:
            ...
            for tr in trs:
                ...
                coreTeam.position = try_and_text(
                    "variable[2].xpath('.//text()')[0]", tds)
    ...
    def html_parse_entBusiness(self, index):
        ...
        if trs:
            ...
            key = self.search_name
            # 序号	产品名称	成立日期	当前融资轮次	产品标签	所属地	产品介绍
            for tr in trs:
                tds = tr.xpath('./td')
                insert_value = ""
                # 产品名称
                entBusiness.businessName = try_and_text(
                    "variable[1].xpath('.//text()')[-1]", tds)

                entBusiness.register_date = try_and_text(
                    "variable[2].xpath('./text()')[0]", tds)

                # 当前融资轮次 Financing rounds
                entBusiness.financing_rounds = try_and_text(
                    "variable[3].xpath('./text()')[0]", tds)

                # 产品标签
                entBusiness.businessQuale = try_and_text(
                    "variable[4].xpath('.//text()')[-1]", tds)
                # 所属地
                entBusiness.home_location = try_and_text(
                    "variable[5].xpath('./text()')[-1]", tds)
                # 产品介绍
                entBusiness.businessInfo = try_and_text(
                    "variable[3].xpath('.//text()')[0]", tds)
    ...
    def html_parse_investEvent(self, index):
        ...
        if trs:
            ...
            key = self.search_name
            # 序号	投资日期	参与轮次	投资方	投资金额	产品名称	产品标签	所属地	简介
            for tr in trs:
                insert_value = ""
                tds = tr.xpath('./td')
                investEvent.touziDate = try_and_text(
                    "variable[1].xpath('string(.)')", tds)
                investEvent.touziRound = try_and_text(
                    "variable[2].xpath('string(.)')", tds)
                investEvent.touziMoney = try_and_text(
                    "variable[4].xpath('string(.)')", tds)
                investEvent.touziEnt = ','.join(try_and_text(
                    "variable[3].xpath('//text()')", tds))
                investEvent.touziProduct = try_and_text(
                    "(variable[5].xpath('.//a/text()'))[0]", tds)
                investEvent.touziArea = try_and_text(
                    "variable[7].xpath('string(.)')", tds)
                investEvent.touziIndustry = try_and_text(
                    "variable[6].xpath('.//a/text()'))[0]", tds)
                investEvent.touziBusiness = try_and_text(
    ...
    def html_parse_jpInfo(self, index):
        ...
        if trs:
            ...
            for tr in trs:
                insert_value = ""
                tds = tr.xpath('./td')
                jpInfo.jpProduct = try_and_text(
                    "variable[1].xpath('.//a/text()')[0]", tds)
                jpInfo.jpArea = try_and_text(
                    "variable[6].xpath('.//text()')[0]", tds)
                jpInfo.jpRound = try_and_text(
                    "variable[4].xpath('.//text()')[0]", tds)
                jpInfo.jpIndustry = try_and_text(
                    "variable[5].xpath('.//a/text()')[0]", tds)
                jpInfo.jpBusiness = try_and_text(
                    "variable[7].xpath('.//text()')[0]", tds)
                jpInfo.jpDate = try_and_text(
                    "variable[4].xpath('.//text()')[0]", tds)
                jpInfo.jpValue = try_and_text(
                    "variable[3].xpath('.//text()')[0]", tds)
    ...
    def html_parse_gsj(self, index):
        ...
        if index == 1 and not isinstance(self.selector, int):
            ...
            for tr in trs:
                ...
                tds = tr.xpath('./td')
                # 许可书文编号
                gsjInfo.licenseDocNum = try_and_text(
                    "variable[1].xpath('./text()')[0]", tds)
                # 许可文件名称
                gsjInfo.licenseDocName = try_and_text(
                    "variable[2].xpath('./text()')[0]", tds)
                # 有效期自

                gsjInfo.validityBegin = try_and_text(
                    "variable[3].xpath('./text()')[0]", tds)
                # 有效期至

                gsjInfo.validityEnd = try_and_text(
                    "variable[4].xpath('./text()')[0]", tds)
                # 许可机关

                gsjInfo.licenseAuthority = try_and_text(
                    "variable[5].xpath('./text()')[0]", tds)
                # 许可内容
                gsjInfo.licenseContent = try_and_text(
                    "variable[6].xpath('./text()')[0]", tds)
    ...
    def html_parse_xyzg(self, index):
        ...
        if trs:
            ...
            for tr in trs:
                ...
                tds = tr.xpath('./td')
                # 行政许可文书号
                dxxkInfo.licenseDocNum = try_and_text(
                    "variable[1].xpath('.//text()')[0]", tds)
                # 许可决定机关
                dxxkInfo.licenseAuthority = try_and_text(
                    "variable[2].xpath('.//text()')[0]", tds)
                # 许可决定日期
                dxxkInfo.licenseDate = try_and_text(
                    "variable[3].xpath( './/text()')[0]", tds)
                # 详情
                text_info = try_and_text(
    ...
    def html_parse_certificateInfo(self, index):
        ...
        if trs:
            ...
            for tr in trs:
                insert_value = ""
                tds = tr.xpath('./td')
                certificateInfo.certificateType = try_and_text(
                    "variable[1].xpath('./span/text()')[0]", tds)
                certificateInfo.certificateNum = try_and_text(
                    "variable[2].xpath('./span/text()')[0]", tds)
                certificateInfo.sendDate = try_and_text(
                    "variable[3].xpath('./span/text()')[0]", tds)
                certificateInfo.offDate = try_and_text(
    ...
    def html_parse_entWechat(self, index):
        ...
        if index:
            ...
            trs = self.selector.xpath(
                '//div[@id="_container_wechat"]/table/tbody/tr')
        ...
        if trs:
            ...
            for tr in trs:
                insert_value = ""
                entWeChat.mp_name = try_and_text(
                    "variable.xpath('./td')[1].xpath('.//span/text()')[1]", tr)
                entWeChat.mp_number = try_and_text(
                ...
                insert_sql = create_insert_sql(
                    entWeChat.table_name, entWeChat.column_name, len(
    ...
    def html_parse_buyInfo(self, index):
        ...
        if index == 1 and not isinstance(self.selector, int):
            ...
            trs = self.selector.xpath(
                '//div[@id="_container_purchaselandV2"]/table/tbody/tr')
        ...
        if trs:
            ...
            for tr in trs:
                ...
                # 签订日期
                buyInfo.gdSignDate = try_and_text(
                    "variable[6].xpath('./text()')[0]", tds)
                # 土地坐落
                where = try_and_text(
                    "variable[1].xpath('./text()')[0]", tds)
    ...
    def html_parse_dxxk(self, index):
        ...
        if trs:
            ...
            for tr in trs:
                ...
                tds = tr.xpath('./td')
                # 许可证号
                dxxkInfo.licenseKey = try_and_text(
                    "variable[1].xpath('./text()')[0]", tds)
                # 业务范围
                dxxkInfo.businessSphere = try_and_text(
                    "variable[2].xpath('./text()')[0]", tds)
                # 是否有效
                dxxkInfo.available = try_and_text(
                    "variable[3].xpath('./text()')[0]", tds)
                # 详情
                text_info = try_and_text(
                    "variable[4].xpath('./script/text()')[0]", tds)
    ...
    def html_parse_zzsyr(self, index):
        # TODO 这是个异步抓取页面，
        # https://www.tianyancha.com/company/holder_holding_analysis.xhtml?id=22822&_=1557886017705
        # GET
        ...
        if trs:
            ...
            for tr in trs:
                ...
                tds = tr.xpath('./td')
                # 最终受益人名称
                benefitPerson.beneficiaryName = try_and_text(
                    "variable[1].xpath('/span/a/text()')[0]", tds)
                # 持股比例
                benefitPerson.shareholderProportion = try_and_text(
                    "variable[2].xpath('./span/text()')[0]", tds)
                # 股权链
                benefitPerson.equityChain = try_and_text(
    ...
    def html_parse_sjkzq(self, index):
        # TODO
        # 这是个异步抓取页面，https://www.tianyancha.com/company/holder_holding_analysis.xhtml?id=22822&_=1557886017705
        ...
        if trs:
            ...
            for tr in trs:
                ...
                tds = tr.xpath('./td')
                # 控股企业名称
                holdingInfo.holdingName = try_and_text(
                    "variable[1].xpath('.//a/txext()')[0]", tds)
                # 投资比例 //*[@id="_container_companyholding"]/table/tbody/tr[1]/td[3]/span
                holdingInfo.investProportion = try_and_text(
                    "variable[2].xpath('.//text()')[0]", tds)
                # 投资链
                holdingInfo.equityChain = try_and_text(
                    "variable[3].xpath('string(.)')", tds)
