...
if __name__ == '__main__':
    # single_redis_remote = Redis_Util(host='10.10.82.12', port=6379, db='')
    single_redis_remote = Redis_Util(host='10.0.32.85', port=6379, db='')
    # cookies = single_redis_remote.server.hgetall('cookies')
    # print(len(cookies))
    # for i, v in cookies.items():
    #     print(type(i), type(v))
    #     print(i)
    #     single_redis_remote.server.hdel('cookies',i)
    ...
    #         v=v.decode()
    #
    #     single_redis_remote.server.hset('cookies',i,v)
    # single_redis_local.server.hset('cookies', k, v)
    ...
    #
    #     single_redis_local.server.hset('not_xiong_an',name,1)
    users = single_redis_remote.server.llen('users')
    u = []
    for i in range(0, users):
        i = single_redis_remote.server.rpop('users')
        if isinstance(i, bytes):
            i = i.decode()
            i = i.replace('"', '').replace("'", "").replace('\\', '').replace('b', '')
            if i not in u:
                u.append(i)
                print(i)
                single_redis_remote.server.lpush('users', i)
