...
class TycDetailParse(object):
    ...
    def html_parse_baseinfo(self):
        ...
        if top:
            ...
            try:
                ...
                email = 'NA'
            baseinfo.email = email.replace(r'"', r'').replace(r'[', r'').replace(r']', r'')
            ...
            if table:
                ...
                if trs:
                    ...
                    for tr in trs:
                        ...
                        # print(shareholderInfo.__dict__)
    ...
    def html_parse_investInfo(self, index):
        ...
        if trs:
            ...
            for tr in trs:
                ...
                investInfo.investRatio = try_and_text("variable[3].xpath('.//text()')[0]", tds)
    ...
    def html_parse_year_jbxx(self, year_selector, year):
        ...
        # 获得大标签
        if year_selector:
            year_selector = year_selector.replace(r'企业基本信息', 'year_basic_info')
    ...
    def html_parse_year_wzhwdxx(self, year_selector, year):
        ...
        year_selector = year_selector.replace(r'网站或网店信息', 'year_wangzhan')
        if year_selector:
            year_selector = etree.HTML(year_selector)
    ...
    def html_parse_year_gdczxx(self, year_selector, year):
        ...
        gdcz = TycYearGdczxx()
        # 获得分支机构大标签
        year_selector = year_selector.replace(r'股东及出资信息', 'year_gudongchuzi')
        if year_selector:
            year_selector = etree.HTML(year_selector)
    ...
    def html_parse_year_zczk(self, year_selector, year):
        ...
        year_selector = year_selector.replace(
            r'企业资产状况信息', 'year_ent_money_info')
        if year_selector:
            year_selector = etree.HTML(year_selector)
        info = r'企业资产状况信息'
        trs = year_selector.xpath(
    ...
    def html_parse_year_dwtz(self, year_selector, year):
        ...
        year_selector = year_selector.replace(
            r'对外投资信息', 'year_outbound_company')
        if year_selector:
            year_selector = etree.HTML(year_selector)
    ...
    def html_parse_shixinren(self, index):
        ...
        if root_div:
            ...
            for tr in trs:
                ...
                # TODO:失信人的唯一字段暂未找到
                unique_field = ['ent_name', flss.ent_name]  # 该模块中唯一值字段名和值
                check_parse(flss, add_result, unique_field)
    ...
    def html_parse_financeHistory(self):
        ...
        if trs:
            ...
            for tr in trs:
                insert_value = ""
                tds = tr.xpath('./td')
                flss.financeDate = try_and_text("variable[1].xpath('string(.)')", tds)
                flss.event_date = try_and_text("variable[2].xpath('string(.)')", tds)
                flss.financeRound = try_and_text("variable[4].xpath('string(.)')", tds)
                flss.financeValue = try_and_text("variable[5].xpath('string(.)')", tds)
                flss.financeMoney = try_and_text("variable[3].xpath('string(.)')", tds)
                flss.financeRatio = try_and_text("variable[6].xpath('string(.)')", tds)
    ...
    def html_parse_investEvent(self, index):
        ...
        if trs:
            ...
            for tr in trs:
                insert_value = ""
                tds = tr.xpath('./td')
                flss.touziDate = try_and_text("variable[1].xpath('string(.)')", tds)
                flss.touziRound = try_and_text("variable[2].xpath('string(.)')", tds)
                flss.touziMoney = try_and_text("variable[3].xpath('string(.)')", tds)
                flss.touziEnt = try_and_text("variable[4].xpath('string(.)')", tds)
                flss.touziProduct = try_and_text("(variable[5].xpath('.//a/text()'))[0]", tds)
    ...
    def html_parse_jpInfo(self, index):
        ...
        if trs:
            ...
            for tr in trs:
                insert_value = ""
                tds = tr.xpath('./td')
                flss.jpProduct = try_and_text("variable[1].xpath('.//a/text()')[0]", tds)
                flss.jpArea = try_and_text("variable[2].xpath('.//text()')[0]", tds)
                flss.jpRound = try_and_text("variable[3].xpath('.//text()')[0]", tds)
                flss.jpIndustry = try_and_text("variable[4].xpath('.//text()')[0]", tds)
        ...
        if index == 1 and not isinstance(self.selector, int):
            ...
            if trs:
                ...
                for tr in trs:
                    ...
                    tds = tr.xpath('./td')
                    # 许可书文编号
                    flss.licenseDocNum = try_and_text("variable[1].xpath('./text()')[0]", tds)
                    # 许可文件名称
                    flss.licenseDocName = try_and_text("variable[2].xpath('./text()')[0]", tds)
                    # 有效期自
                    flss.validityBegin = try_and_text("variable[3].xpath('./text()')[0]", tds)
                    # 有效期至
                    flss.validityEnd = try_and_text("variable[4].xpath('./text()')[0]", tds)
                    # 许可机关
                    flss.licenseAuthority = try_and_text("variable[5].xpath('./text()')[0]", tds)
                    # 许可内容
    ...
    def html_parse_xyzg(self, index):
        ...
        if trs:
            ...
            for tr in trs:
                ...
                flss.licenseDocNum = try_and_text("variable[1].xpath('.//text()')[0]", tds)
                # 许可决定机关
                flss.licenseAuthority = try_and_text("variable[2].xpath('.//text()')[0]", tds)
                # 许可决定日期
                flss.licenseDate = try_and_text("variable[3].xpath( './/text()')[0]", tds)
                # 详情
    ...
    def html_parse_certificateInfo(self, index):
        ...
        if trs:
            ...
            for tr in trs:
                insert_value = ""
                tds = tr.xpath('./td')
                flss.certificateType = try_and_text("variable[1].xpath('./span/text()')[0]", tds)
                flss.certificateNum = try_and_text("variable[2].xpath('./span/text()')[0]", tds)
                flss.sendDate = try_and_text("variable[3].xpath('./span/text()')[0]", tds)
                flss.offDate = try_and_text("variable[4].xpath('./span/text()')[0]", tds)
    ...
    def html_parse_buyInfo(self, index):
        ...
        if trs:
            ...
            for tr in trs:
                ...
                tds = tr.xpath('./td')
                # 签订日期
                flss.gdSignDate = try_and_text("variable[6].xpath('./text()')[0]", tds)
                # 土地坐落
                ...
                flss.gdNum = CURRENT_VERSION_NULL
                flss.gdActDate = CURRENT_VERSION_NULL
    ...
    def html_parse_dxxk(self, index):
        ...
        if trs:
            ...
            for tr in trs:
                ...
                tds = tr.xpath('./td')
                # 许可证号
                flss.licenseKey = try_and_text("variable[1].xpath('./text()')[0]", tds)
                # 业务范围
                flss.businessSphere = try_and_text("variable[2].xpath('./text()')[0]", tds)
                # 是否有效
    ...
    while True:
        ...
        if detail_info:
            ...
            if NEXT_PAGE_DICT[search_name]:
                ...
                ll = {'北京百度网讯科技有限公司': {'tyc_zscq_wzba': '网站备案', 'tyc_qybj_dwtz': '对外投资', 'tyc_jyfx_xzcf': '行政处罚 工商局',
                                       'tyc_qybj_bgjl': '变更记录', 'tyc_sffx_ktgg': '开庭公告', 'tyc_sffx_flss': '法律诉讼',
                                       'tyc_sffx_fygg': '法院公告',
                                       'tyc_jyfx_gqcz': '股权出质', 'tyc_qyfz_hxtd': '核心团队', 'tyc_qyfz_qyyw': '企业业务',
                                       'tyc_qyfz_tzsj': '投资事件',
                                       ...
                                       'tyc_zscq_sbxx': '商标信息', 'tyc_zscq_zl': '专利', 'tyc_zscq_zzq': '软件著作权',
                                       'tyc_zscq_zpzzq': '作品著作权'}}
                not_parse_table_names = str(list(NEXT_PAGE_DICT[search_name].keys()))[1:-1]  # 得出未解析的模块名称拼接字符串
                next_page_parse = CheckResult()
                next_page_parse.company_name = search_name
                next_page_parse.add_time = func.now()
                next_page_parse.different_reason = '分页未解析模块:' + str(list(NEXT_PAGE_DICT[search_name].values()))[1:-1]
                next_page_parse.table_name = not_parse_table_names
