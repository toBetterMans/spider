...
class Redis_Util(object):
    ...
    def put_phone(self):
        ...
        if status:
            ...
            if users_length > 200:
                print('phones length={}'.format(users_length))
                self.del_lock(lock_uid, sec)
                return
    ...
    def put_11315_update(self):
        ...
        if status:
            ...
            try:
                ...
                for i in max2:
                    if i and i[1]:
                        self.server.lpush('11315_updates', str(i[0])+';'+i[1])
                    else:
                        print(i)
                ...
                self.del_lock(lock_uid, sec)
                print(e)
single_reids=Redis_Util(host='10.0.32.85', port=6379, db='')
if __name__ == '__main__':
    # single_reids_local = Redis_Util(host='127.0.0.1', port=6379, db='')
    single_reids_remote = Redis_Util(host='10.0.32.85', port=6379, db='')
    # cookies=single_reids_local.server.hgetall('cookies')
    # for k,v in cookies.items():
    #     single_reids_remote.server.hset('cookies',k,v)
    single_reids_remote.put_11315_update()
    # bf.put_page()
    # bf.put_parse()
    # import xlrd
    ...
    #     single_reids_local.server.hset('not_xiong_an',name,1)
