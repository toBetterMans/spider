...
from setting import REDIS_HOST, REDIS_DB, REDIS_PORT,REDIS_PASSWORD
...
class RedisUtil(object):
    ...
    def __init__(self, blockNum=1, key='bloomfilter'):
        ...
        '''
        self.server = redis.Redis(host=REDIS_HOST, port=REDIS_PORT,password=REDIS_PASSWORD, db=REDIS_DB)
        ...
        if status:
            ...
            try:
                ...
                if results:
                    ...
                    # single_oracle.oracle_update(
                    #     "update config set over = 1, over_time = sysdate where id = {}".format(tablename_condition[2]))
                    # return
