...
from cpca import *
import os
from tyc_detail.db import single_oracle
        ...
        for re in res:
            ...
            if re[0]:
                ...
                for addr in df.index:
                    ...
                    if addr_1 :
                        if (addr_3 == '雄县' or addr_3 == '容城县' or addr_3 == '安新县',addr_3 == '容城' or addr_3 == '安新') and addr_1 == '河北省':
                            # 测试 70
                            update_sql = "update tyc_qybj_jbxx set province ='{sheng}',city='{shi}',county='{xian}' where company_name='{comp_name}'".format(sheng=addr_1,shi=addr_2,xian=addr_3,comp_name=re[1])
                            # 本地
...
def business_term(res):
    for re in res:
        ...
        sql = "update tyc_qybj_jbxx set business_term_begin ='{begin}', business_term_end = '{end}' where company_name='{name}'".format(begin=begin, end=end, name=re[1])
        print(sql)
