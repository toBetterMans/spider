...
class RedisUtil(object):
    ...
    def put_flag(self):
        '''
        从配置表config中取出flag放入redis
        :return:
        '''
        try:          
            flag_condition = self.get_configs_by_tablename('put_flag')
            flag = flag_condition[0]

            self.server.set('11315_increased_flag', flag)
            print('将flag放入队列')
        ...
        if status:
            ...
            try:
                ...
                if results[0][0]:
                    ...
                    max_num = single_oracle.oracle_find_by_param_all('select max(company_number) from company_11315')
                    
                    if max_num[0][0] > results[0][0]:
                        single_oracle.execute('delete from company_11315 where company_number > {}'.format(results[0][0]))
